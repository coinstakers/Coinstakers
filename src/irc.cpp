...
void ThreadIRCSeed()
{
    ...
    RenameThread("Coinstakers-ircseed");
...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #CoinstakersTEST\r");
            Send(hSocket, "WHO #CoinstakersTEST\r");
        } else
        {
	    LogPrintf("JOIN #Coinstakers\n");
            // randomly join #Coinstakers00-#Coinstakers05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Coinstakers%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Coinstakers%02d\r", channel_number).c_str());
        };
