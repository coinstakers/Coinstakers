...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Coinstakers"))
        return false;
...
{
    // Convert Coinstakers:// to Coinstakers:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Coinstakers://"))
    {
        uri.replace(0, 12, "Coinstakers:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Coinstakers.lnk";
}
...
{
    return GetAutostartDir() / "Coinstakers.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Coinstakers-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Coinstakers-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Coinstakers-Qt"));
