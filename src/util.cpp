...
int nAnonymizeCoinstakersAmount = 500;
...
#else
    const char* pszModule = "Coinstakers";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Coinstakers
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Coinstakers
    // Mac: ~/Library/Application Support/Coinstakers
    // Unix: ~/.Coinstakers
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Coinstakers";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Coinstakers.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) //Coinstakers.conf doesn't exist
    {
        createConf();
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Coinstakersd.pid"));
