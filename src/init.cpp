...
void Shutdown()
{
    ...
    RenameThread("Coinstakers-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Coinstakers.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Coinstakersd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeCoinstakersamount=<n> " + _("Keep N Coinstakers anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Coinstakers is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Coinstakers is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Coinstakers version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Coinstakers server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Coinstakers") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Coinstakers to complete") << "\n";
    ...
    nAnonymizeCoinstakersAmount = GetArg("-anonymizeCoinstakersamount", 0);
    if(nAnonymizeCoinstakersAmount > 999999) nAnonymizeCoinstakersAmount = 999999;
    if(nAnonymizeCoinstakersAmount < 2) nAnonymizeCoinstakersAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeCoinstakersAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize Coinstakers Amount %d\n", nAnonymizeCoinstakersAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1Coinstakers+1000 == (.1Coinstakers+100)*10
       10Coinstakers+10000 == (1Coinstakers+1000)*10
    */
