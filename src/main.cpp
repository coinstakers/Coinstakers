...
const string strMessageMagic = "Coinstakers Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Rather not work on nonstandard transactions (unless -testnet)
    //alot of Coinstakers transactions seem non standard, its a bug so we have to accept these, the transactions have still been checekd to be valid and unspent.
    string reason;
...
int64_t GetProofOfWorkReward(int64_t nFees, unsigned int nHeight)
{
    ...
    if(nHeight == 1)
    {
       nSubsidy = 545454545 * COIN;
    }

    nSubsidy >>= (nHeight / 1025280); 
...
#ifdef ENABLE_WALLET
// Coinstakers: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Coinstakers-loadblk");
